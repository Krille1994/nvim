return {
	...
	opts = {
		...
		on_attach = function(bufnr)
			...
			map("n", "<leader>gq", function() gs.setqflist("all") end, "Hunks → quickfix")
			map("n", "<leader>gl", gs.setloclist, "Hunks → loclist")
			map("n", "<leader>gT", gs.toggle_signs, "Toggle signs")
			map("n", "<leader>gL", gs.toggle_linehl, "Toggle line hl")
			map("n", "<leader>gN", gs.toggle_numhl, "Toggle number hl")
